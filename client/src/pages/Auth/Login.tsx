...
const Login = () => {
  const { handleInputChange, handleSubmit, fields, loading, error } =
    useAuthLogin();
          ...
          >
            ...
            {fields.map((field, index) => (
              <InputField
                ...
                error={field.error} // Pass error for individual field validation
              />
            ))}
            ...
            <button
              type="submit"
              disabled={loading || fields.some((f) => f.error)} // Disable if loading or errors
              className={`flex items-center justify-center w-full py-3 rounded-full font-medium transition ${
                loading || fields.some((f) => f.error)
